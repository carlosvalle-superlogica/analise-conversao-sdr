        ...
        if pipeline_sel == "Aquisições":
            ...
            if pagina_sel == "📊 Dashboard Geral":
                ...
                if st.session_state['perfil'] == "master":
                    ...
                    t_sdr_funil = (
                        s_l
                        .merge(s_c, on='Filtro_SDR', how='outer')
                        .merge(s_a, on='Filtro_SDR', how='outer')
                        .merge(s_r, on='Filtro_SDR', how='outer')
                        .merge(s_f, on='Filtro_SDR', how='outer')
                        .fillna(0)
                    )
                    ...
                    st.dataframe(
                        t_sdr_funil.rename(columns={'Filtro_SDR': 'SDR Responsável'}).sort_values(by='Leads', ascending=False),
                    ...
                    with col_ef1:
                        ...
                        st.dataframe(
                            t_sdr_q.rename(columns={'Filtro_SDR': 'SDR Responsável'}).sort_values(by='Leads', ascending=False),
                            use_container_width=True, hide_index=True
                        )
                    ...
                    with col_ef2:
                        ...
                        st.dataframe(
                            t_cl.rename(columns={'Filtro_Closer': 'Closer Responsável'}).sort_values(by='Ocorridos', ascending=False),
                            use_container_width=True, hide_index=True
                        )
            ...
            # --------------------------------------------------------------------------
                ...
                if col_estado in df_base.columns:
                    ...
                    # Extrai os estados únicos dinamicamente dos dados filtrados
                    estados_disponiveis = sorted(df_mapa_local[col_estado].unique().tolist())
                    ...
                    # Cria as máscaras de datas localmente para o dataframe reduzido
                    ...
                    map_r = df_mapa_local[mR_mapa].groupby(col_estado).size().reset_index(name='RO (Reunião Ocorrida)')
                    map_f = df_mapa_local[mF_mapa].groupby(col_estado).size().reset_index(name='Fechados/Pagos')
            ...
            # --------------------------------------------------------------------------
                ...
                # ------ NOVO: Produtos de Interesse do Lead ------
                ...
                if col_interesse in df_base.columns:
                    ...
                    if not df_int.empty:
                        ...
                        st.dataframe(t_int.rename(columns={col_interesse: 'Produto'}).sort_values(by='Leads', ascending=False), use_container_width=True, hide_index=True)
                    else:
                        st.info("Nenhum lead com 'Produto de Interesse' preenchido no período atual.")
                ...
                if col_prod in df_base.columns:
                    ...
                    contagem_prod['% Produto / Reunião'] = contagem_prod['Qtd. Vendida'].apply(
                        lambda x: f"{(x / tR * 100):.1f}%" if tR > 0 else "0.0%"
                    )
                    ...
                    df_sync['Conversao_Limpa'] = df_sync.apply(
                        lambda r: f"{(r['Fechados'] / r['Ocorridos'] * 100):.1f}%" if r['Ocorridos'] > 0 else "-", axis=1
                    )
            ...
            # --------------------------------------------------------------------------
                ...
                # Foca estritamente na coorte de leads CRIADOS no período (mL)
                ...
                # TMA (MEDIANA): calculado sobre todos os leads criados no período que possuem ambos os campos
                ...
                if col_1_contato and col_1_contato in df_tma_validos.columns:
                    ...
                    tma_medio_td = pd.NaT
                    if col_1_contato:
                        st.warning(f"A coluna de 1º Contato ('{col_1_contato}') foi identificada mas não está disponível no DataFrame filtrado.")
                    else:
                        st.warning("Nenhuma coluna de 1º Contato encontrada. Verifique se o CSV do HubSpot contém coluna com '1' e 'ontato' no nome.")

                # PERMANÊNCIA (MÉDIA): apenas leads perdidos com motivo 'Sem contato'
                ...
                if total_perdas > 0:
                    ...
                    if total_lixo > 0:
                        ...
                        tabela_lixo['% sobre Todos os Recebidos'] = tabela_lixo['Quantidade'].apply(
                            lambda x: f"{(x / total_recebidos * 100):.1f}%" if total_recebidos > 0 else "0.0%"
                        )
                        tabela_lixo['% sobre Todos os Perdidos'] = tabela_lixo['Quantidade'].apply(
                            lambda x: f"{(x / total_perdas * 100):.1f}%" if total_perdas > 0 else "0.0%"
                        )
                        ...
                        with col_tabela_central:
                            ...
                            st.caption(
                                f"<div style='text-align: center; color: #334155; font-size: 0.95rem;'>"
                                f"<b>Insight de Negócio:</b> Dos {total_recebidos} leads recebidos, "
                                f"<b>{pct_sobre_recebidos:.1f}%</b> foram descartados por falta de contato ou qualificação inicial. "
                                f"Isso significa que o time comercial pôde atuar de verdade em apenas {100-pct_sobre_recebidos:.1f}% da base.</div>",
                                unsafe_allow_html=True
                            )
                    ...
                    pivot_origem = pd.crosstab(
                        df_perdidos['[IS] Origem do lead'],
                        df_perdidos['Motivo de Fechamento Perdido'],
                        margins=True,
                        margins_name='Total (Todas as Origens)'
                    )
                    ...
                    col_auditoria = [
                        ...
                        'Motivo de Fechamento Perdido', 'Motivo de Fechamento Perdido (Sub-motivo)',
                        'Descrição de fechamento perdido', 'Repescagem_Limpa'
                    ]
                    st.dataframe(
                        df_perdidos[col_auditoria].fillna('-').rename(columns={
                            'Responsavel_Papel': 'Responsável Pela Perda',
                            'Repescagem_Limpa': 'É Repescagem?'
                        }),
                        use_container_width=True, hide_index=True
                    )
