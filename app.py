...
if not st.session_state['autenticado']:
    ...
    try:
        # ==============================================================================
        # FUNÇÃO MATEMÁTICA DE FORMATAÇÃO DE TEMPO (NOVO PADRÃO: 0h 5min)
        # ==============================================================================
        def formata_tempo(segundos):
            if pd.isna(segundos) or segundos < 0: return "Sem dados"
            try:
        ...
        # --- CÁLCULO DE TEMPO EM SEGUNDOS PUROS (MODO PLANILHA) ---
        ...
        if pipeline_sel == "Aquisições":
            ...
            # --------------------------------------------------------------------------
                ...
                # TMA EXATO DA REGRA DO CARLOS: Usa os segundos pré-calculados lá em cima!
                tma_validos = df_base.loc[mL, 'TMA_Segundos']
                tma_validos = tma_validos[tma_validos >= 0]
                tma_medio_sec = tma_validos.mean() if not tma_validos.empty else np.nan
                ...
                if total_perdas > 0:
                    ...
                    perm_validos = perm_validos[perm_validos >= 0]
                    perm_media_sec = perm_validos.mean() if not perm_validos.empty else np.nan
                else:
                ...
                ct1.metric(
                    ...
                    "Média de todos os leads do período", 
                    delta_color="off"
                )
                ct2.metric(
