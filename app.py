...
if not st.session_state['autenticado']:
    ...
    try:
        ...
        if st.session_state['perfil'] == "master":
            menu_opcoes = ["📊 Dashboard Geral", "🗺️ Mapa Geográfico", "📦 Produtos / Closer's / VC", "📊 Comparação e Gráficos", "💰 Receita", "❌ Perdidos", "⚙️ Configurações"]
        else:
            menu_opcoes = ["📊 Dashboard Geral", "🗺️ Mapa Geográfico", "📊 Comparação e Gráficos"]
        ...
        if pipeline_sel == "Aquisições":
            ...
            # --------------------------------------------------------------------------
                ...
                if col_estado in df_base.columns:
                    ...
                    if len(df_mapa_local) > 0:
                        ...
                        st.warning("Nenhum lead com sigla de estado (2 letras) válida encontrado no período e filtros aplicados.")
                else:
            ...
            # --------------------------------------------------------------------------
                ...
                # --- NOVO: Tabela de Produtos de Interesse do Lead ---
                ...
                if col_interesse in df_base.columns:
                    ...
                    if not df_int.empty:
                        # Associa o lead isolado às máscaras de data originais
                        ...
                        st.dataframe(
                            t_int.rename(columns={col_interesse: 'Produto de Interesse'}).sort_values(by='Leads', ascending=False), 
            ...
            # --------------------------------------------------------------------------
                ...
                if total_perdas > 0:
                    ...
                    if 'Perm_Timedelta' in df_perdidos.columns:
                        ...
                        df_perdidos['Tempo no Funil'] = 'Sem dados'

                    if 'TMA_Timedelta' in df_perdidos.columns:
                        df_perdidos['TMA do Lead'] = df_perdidos['TMA_Timedelta'].apply(formata_tempo)
                    else:
                        df_perdidos['TMA do Lead'] = 'Sem dados'
