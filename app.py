...
if not st.session_state['autenticado']:
    ...
    try:
        # ==============================================================================
        # FUNÇÃO DE FORMATAÇÃO DE TEMPO (ESTILO PLANILHA: Xh Ymin)
        # ==============================================================================
        ...
        if pipeline_sel == "Aquisições":
            ...
            # --------------------------------------------------------------------------
                ...
                # TMA EXATO DA REGRA DO CARLOS (Com formatação idêntica ao Excel: 0h 5min)
                valid_tma = df_base.loc[mL, 'TMA_Segundos']
                valid_tma = valid_tma[valid_tma >= 0]
                tma_medio_sec = valid_tma.mean() if not valid_tma.empty else np.nan
                
                # PERMANÊNCIA: Apenas leads perdidos por 'Sem contato'
                if total_perdas > 0:
                    mask_sc = df_perdidos['Motivo de Fechamento Perdido'] == 'Sem contato'
                    valid_perm = df_perdidos.loc[mask_sc, 'Perm_Segundos']
                    valid_perm = valid_perm[valid_perm >= 0]
                    perm_media_sec = valid_perm.mean() if not valid_perm.empty else np.nan
                else:
                    perm_media_sec = np.nan
                
                ct1, ct2 = st.columns(2)
                ct1.metric(
                    "TMA Médio (Data de Criação ➔ 1º Contato)", 
                    formata_tempo(tma_medio_sec), 
                    "Média real formatada em Horas e Minutos", 
                    delta_color="off"
                )
                ct2.metric(
                    "Permanência Média (Apenas motivo 'Sem Contato')", 
                    formata_tempo(perm_media_sec), 
                    "Data Criação ➔ Data Perdido", 
                    delta_color="off"
                )
