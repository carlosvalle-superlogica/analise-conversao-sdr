...
try:
    ...
    # Preenchemos os vazios com texto para o filtro não deletá-los por acidente
    ...
    # ---> NOVA ADIÇÃO: EFICIÊNCIA GERAL (IMPACTO NO LEAD)
    st.divider()
    ...
    with col_ef1:
        st.write("**SDR: Lead x Ocorrido e Lead x Fechado**")
    ...
    with col_ef2:
        st.write("**Closer: Lead x Ocorrido e Lead x Fechado**")
        cl_ef_l = df_base[mask_L & mask_has_closer].groupby('Filtro_Closer').size().reset_index(name='Leads')
        ...
        ef_cl = cl_ef_l.merge(cl_ef_r, on='Filtro_Closer', how='outer').merge(cl_ef_f, on='Filtro_Closer', how='outer').fillna(0)
        ef_cl = ef_cl.rename(columns={'Filtro_Closer': 'Closer Responsável'})
        
        ef_cl['Lead x Ocorrido (%)'] = ef_cl.apply(lambda row: f"{(row['Ocorridos']/row['Leads']*100):.1f}%" if row['Leads'] > 0 else "-", axis=1)
        ef_cl['Lead x Fechado (%)'] = ef_cl.apply(lambda row: f"{(row['Fechados']/row['Leads']*100):.1f}%" if row['Leads'] > 0 else "-", axis=1)
        
        col_ef_cl = ['Closer Responsável', 'Leads', 'Ocorridos', 'Fechados', 'Lead x Ocorrido (%)', 'Lead x Fechado (%)']
        st.dataframe(ef_cl[col_ef_cl].sort_values(by='Leads', ascending=False), use_container_width=True, hide_index=True)
