                ...
                if st.session_state['perfil'] == "master":
                    ...
                    with col_ef3:
                        ...
                        if col_cs in df_base.columns:
                            ...
                            cs_l = df_base[mL & mask_cs].groupby(col_cs).size().reset_index(name='Indicações')
                            ...
                            t_cs['Indicações'] = t_cs['Indicações'].astype(int)
                            t_cs['Reuniões Ocorridas'] = t_cs['Reuniões Ocorridas'].astype(int)
                            t_cs['Fechados e Pagos'] = t_cs['Fechados e Pagos'].astype(int)
                            
                            t_cs['Lead ➔ Ocorrido (%)'] = t_cs.apply(lambda r: f"{(r['Reuniões Ocorridas']/r['Indicações']*100):.1f}%" if r['Indicações']>0 else "0.0%", axis=1)
                            t_cs['Lead ➔ Fechado (%)'] = t_cs.apply(lambda r: f"{(r['Fechados e Pagos']/r['Indicações']*100):.1f}%" if r['Indicações']>0 else "0.0%", axis=1)
                            
                            st.dataframe(t_cs.rename(columns={col_cs: 'CS Responsável'}).sort_values(by='Indicações', ascending=False), use_container_width=True, hide_index=True)
